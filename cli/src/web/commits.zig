...
fn loadCommits(allocator: Allocator, repo: Repo, ref: []const u8, path: []const u8) ![]CommitListEntry {
    ...
    const raw_opt = if (path.len == 0)
        try gitMaybe(allocator, repo, &.{ "log", "-50", "--date=short", format, "--end-of-options", ref }, git.max_git_output)
    else blk: {
        const pathspec = try std.fmt.allocPrint(allocator, ":(top){s}", .{path});
        defer allocator.free(pathspec);
        break :blk try gitMaybe(allocator, repo, &.{ "log", "-50", "--date=short", format, "--end-of-options", ref, "--", pathspec }, git.max_git_output);
    };
